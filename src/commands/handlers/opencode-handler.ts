...
/**
 * Map cohe provider name to OpenCode provider name
 */
...
export function configureOpenCodeWithEnvVars(
  ...
  const config = loadOpenCodeConfig();
  const envKey = `COHE_${account.provider.toUpperCase()}_API_KEY`;
  const envGroupId =
    account.provider === "minimax"
      ? `COHE_${account.provider.toUpperCase()}_GROUP_ID`
      : null;
...
export async function handleOpenCodeSetup(silent = false): Promise<void> {
  try {
    ...
    // This allows cohe to control credentials via env vars without rewriting config
    config.provider = {
      minimax: {
        npm: "@ai-sdk/anthropic",
        options: {
          baseURL: "{env:COHE_BASE_URL}",
          apiKey: "{env:COHE_API_KEY}",
        },
        models: {
          "MiniMax-M2.5": {
            name: "MiniMax-M2.5",
    ...
    if (!silent) {
      ...
      info("  • COHE_PROVIDER_NPM - NPM package for provider");
      info("  • COHE_BASE_URL - API base URL");
      info("  • COHE_API_KEY - API key");
      info("  • COHE_GROUP_ID - Group ID (MiniMax only)");
      info("");
      info("Run 'cohe opencode' to start OpenCode with auto-rotation.");
    }
  } catch (err: unknown) {
    section("OpenCode Setup");
...
export async function handleOpenCodeStatus(silent = false): Promise<void> {
  ...
  if (!silent) {
    ...
    if (!configExists) {
      ...
      info("");
      warning("Run 'cohe opencode setup' to configure OpenCode.");
      return;
    }
...
export function getOpenCodeEnv(account: AccountConfig): Record<string, string> {
  return {
    COHE_PROVIDER_NPM: "@ai-sdk/anthropic",
    COHE_BASE_URL: account.baseUrl,
    COHE_API_KEY: account.apiKey,
    COHE_GROUP_ID: account.groupId || "",
    COHE_PROVIDER: account.provider,
  };
}
