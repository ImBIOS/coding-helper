...
export default class OpenCode extends BaseCommand<typeof OpenCode> {
  ...
  async run(): Promise<void> {
    ...
    if (!accountsConfig.loadConfig().activeAccountId) {
      this.error(
        "No accounts configured. Run 'cohe account add' or 'cohe config' first."
      );
    }
    ...
    if (!activeAccount) {
      ...
      const childEnv: Record<string, string> = {
        ...
        COHE_BASE_URL: providerConfig.baseUrl,
        COHE_API_KEY: providerConfig.apiKey,
      };
    ...
    const childEnv: Record<string, string> = {
      ...process.env,
      COHE_PROVIDER_NPM: opencodeEnv.COHE_PROVIDER_NPM,
      COHE_BASE_URL: opencodeEnv.COHE_BASE_URL,
      COHE_API_KEY: opencodeEnv.COHE_API_KEY,
      COHE_GROUP_ID: opencodeEnv.COHE_GROUP_ID,
      COHE_PROVIDER: opencodeEnv.COHE_PROVIDER,
    };
